...
def get_info_v3(input_first, input_second, input_third):
    url_geo = url_api_geo + 'location=' + input_third + mykey
    ...
    if info['location'][0]:
        ...
        if len(info['location']) == 1:
            ...
            for i in range(0, len(info['location'])):
                ...
                # 查询第二输入是否为市，如果检测为市，直接输出对应
                # 将查询表结果作为子字符串，避免输入中出现“省”“市”等
                if (city['adm2'] in input_first) | (city['adm2'] in input_second):
                    city_info = 1
                    city_id = city['id']
                    district_name = city['name']
                    city_name = city['adm2']
                ...
                # 查询第一输入是否为省，如果监测为省，直接输出对应
                elif input_first in city['adm1']:
                    city_info = 1
                    city_id = city['id']
                    district_name = city['name']
                    city_name = city['adm2']
    ...
    if district_name == city_name:
    ...
    elif district_name in province_name:
        msg = province_name + '\n' + \
            '当前天气：' + current_weather + ' ' + current_temp + '℃  体感温度：' + current_fl_temp + '℃\n' + \
            '空气质量指数：' + current_air + '\n' + \
            '今日天气：' + today_weather + ' ' + today_min_temp + ' - ' + today_max_temp + '℃\n' + \
            '1小时后天气：' + future1h_weather + ' ' + future1h_temp + '℃\n' + \
            '明日天气：' + tomorrow_weather + ' ' + tomorrow_min_temp + ' - ' + tomorrow_max_temp + '℃'
    elif (district_name not in city_name) & (city_name in province_name):
        msg = province_name + district_name + '区\n' + \
            '当前天气：' + current_weather + ' ' + current_temp + '℃  体感温度：' + current_fl_temp + '℃\n' + \
            '空气质量指数：' + current_air + '\n' + \
            '今日天气：' + today_weather + ' ' + today_min_temp + ' - ' + today_max_temp + '℃\n' + \
            '1小时后天气：' + future1h_weather + ' ' + future1h_temp + '℃\n' + \
        ...
        msg = province_name + ' ' + city_name + district_name + '\n' + \
            '当前天气：' + current_weather + ' ' + current_temp + '℃  体感温度：' + current_fl_temp + '℃\n' + \
            '空气质量指数：' + current_air + '\n' + \
            '今日天气：' + today_weather + ' ' + today_min_temp + ' - ' + today_max_temp + '℃\n' + \
            '1小时后天气：' + future1h_weather + ' ' + future1h_temp + '℃\n' + \
...
print('请输入第三位:')
input3 = input()
msg = get_info_v3(input1, input2, input3)
print(msg)
'''
